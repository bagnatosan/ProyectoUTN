                ...
                <div class="flex flex-col justify-center text-xs space-y-1">
                    ...
                    <p class="text-slate-300 font-medium">
                        @if($product->estimated_cost !== null)
                            <span class="text-sm font-bold text-slate-100">${{ number_format($product->estimated_cost, 2) }}</span>
                        @else
                            <span class="text-slate-500 italic">No calculado (sin receta)</span>
                        @endif
                    </p>
                    <p class="text-[10px] text-slate-500 mt-1">
                        Sugerido para venta: <span class="font-semibold text-slate-400">${{ number_format($product->suggested_price ?? 0, 2) }}</span>
                    </p>
                </div>
            </div>
