...
<div class="max-w-2xl mx-auto py-8">
    <div class="border border-slate-800 bg-slate-900/40 backdrop-blur rounded-2xl p-8 shadow-xl">
        ...
        <p class="text-slate-400 mt-2 text-sm">
            Si cambiás el costo unitario, los productos que usen este ingrediente se recalculan automáticamente.
        </p>

        <form action="{{ route('ingredients.update', $ingredient) }}" method="POST" class="mt-8 space-y-5">
            @csrf
            @method('PUT')

            <div>
                <label for="name" class="block text-xs font-semibold uppercase tracking-wider text-slate-400 mb-2">Nombre</label>
                <input id="name" name="name" value="{{ old('name', $ingredient->name) }}" required class="w-full bg-slate-950/80 border border-slate-800 rounded-xl px-4 py-3 text-sm text-white focus:outline-none focus:border-indigo-500">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                <div>
                    ...
                    <select id="unit_measure" name="unit_measure" required class="w-full bg-slate-950/80 border border-slate-800 rounded-xl px-4 py-3 text-sm text-white focus:outline-none focus:border-indigo-500">
                        @foreach(['gr', 'kg', 'ml', 'litro', 'unidad'] as $unit)
                            <option value="{{ $unit }}" {{ old('unit_measure', $ingredient->unit_measure) === $unit ? 'selected' : '' }}>{{ $unit }}</option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <input id="unit_cost" name="unit_cost" type="number" step="0.01" min="0" value="{{ old('unit_cost', $ingredient->unit_cost) }}" required class="w-full bg-slate-950/80 border border-slate-800 rounded-xl px-4 py-3 text-sm text-white focus:outline-none focus:border-indigo-500">
            ...
            <div class="flex justify-end gap-3 pt-4 border-t border-slate-800">
                ...
                <button type="submit" class="px-5 py-2.5 bg-indigo-600 hover:bg-indigo-500 text-white rounded-xl text-sm font-semibold cursor-pointer">Guardar Cambios</button>
            </div>
        </form>
    </div>
</div>
