...
class ReservationUpdated extends Notification implements ShouldQueue
{
    ...
    public function toMail(object $notifiable): MailMessage
    {
        ...
        return (new MailMessage)
            ...
            ->line('Un cliente ha modificado su reserva de **' . $productName . '**.')
            ->line('Nuevos datos: **' . $date . '** a las **' . $time . '**')
            ->when(count($lines) > 0, fn ($msg) => $msg->line('Cambios:')->line(implode("\n", $lines)))
            ->action('Ver detalle', url('/reservations/' . $this->reservation->id . '/detail'))
